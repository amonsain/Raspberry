...
def get_data(interval):
    ...
    if interval == None:
        ...
        curs.execute("SELECT * FROM Pressure WHERE pressure>980 AND timestamp>datetime('now','-%s hours')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT timestamp,max(Pressure) FROM Pressure WHERE pressure>1000 AND timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT timestamp,max(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT timestamp,min(Pressure) FROM Pressure WHERE pressure>1000 AND timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
...
#    curs.execute("SELECT avg(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    rows=curs.execute("SELECT * FROM Pressure WHERE pressure>0 AND timestamp>datetime('new','-1 hour') AND timestamp<=datetime('new')")
...
def validate_input(option_str):
    ...
    if option_str.isalnum():
        ...
        if int(option_str) > 0 and int(option_str) <= 500:
            return option_str
        else:
            return None
    else: 
